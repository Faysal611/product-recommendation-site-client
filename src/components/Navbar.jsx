        ...
        <>
            <div className={`z-10 flex items-center w-screen px-10 justify-between custom-transition sticky top-0 ${isAtTop ? "bg-transparent py-4" : "bg-[#302c78] py-1"}`}>
                ...
                <div className={ `font-medium text-xl flex gap-10 ${isAtTop ? "text-black" : "text-white"}`}>
                    ...
                    {!loading && user ? <>
                        <NavLink className={({ isActive }) => `relative after:absolute after:-bottom-[3px] after:left-0 ${isActive ? "after:h-[6px] after:w-full" : "after:w-0 after:h-[3px]"} cursor-pointer hover:after:w-full after:duration-200 after:bg-[#15c39a]`} to={"/recommendationsForMe"}><button>Recommendations For Me</button></NavLink>
                        <NavLink className={({ isActive }) => `relative after:absolute after:-bottom-[3px] after:left-0 ${isActive ? "after:h-[6px] after:w-full" : "after:w-0 after:h-[3px]"} cursor-pointer hover:after:w-full after:duration-200 after:bg-[#15c39a]`} to={"/myQueries"}><button>My Queries</button></NavLink>
                        <NavLink className={({ isActive }) => `relative after:absolute after:-bottom-[3px] after:left-0 ${isActive ? "after:h-[6px] after:w-full" : "after:w-0 after:h-[3px]"} cursor-pointer hover:after:w-full after:duration-200 after:bg-[#15c39a]`} to={"/myRecommendations"}><button>My Recommendations</button></NavLink>
                    </> : ""}
